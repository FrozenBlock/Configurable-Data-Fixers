...
By contributing to Configurable Data Fixers, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across Configurable Data Fixers.
...
It allows to separate the actual name of the variable and the namespace, `configurableDataFixers`.

Fields marked as `@Unique` must be prefixed with `configurableDataFixers$` or `CONFIGURABLEDATAFIXERS$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

Configurable Data Fixers is licensed under [LGPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "Configurable Data Fixers license file"
